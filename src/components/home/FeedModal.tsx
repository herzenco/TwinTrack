    ...
    <BottomSheet open={open} onClose={handleClose} title={`Feed ${twinName}`}>
      ...
      {!mode && (
        <div className="flex flex-col gap-3">
          <button
            ...
            className="flex items-center gap-4 min-h-[60px] px-5 rounded-xl bg-white/5
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-text-secondary">Formula or breast milk</p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-text-secondary">Start a timed session</p>
            </div>
          </button>
        </div>
      )}
      ...
      {mode === 'bottle' && (
        <div className="flex flex-col gap-5">
          ...
          {/* Feed type */}
          <div>
            <p className="text-xs font-medium text-text-secondary mb-2">Type</p>
            <div className="flex gap-2">
              {(['formula', 'breastmilk'] as FeedType[]).map((t) => (
                <button
                  key={t}
                  onClick={() => setFeedType(t)}
                  className={`flex-1 min-h-[48px] rounded-xl text-sm font-semibold transition-all active:scale-95
                    ${feedType === t
                      ? 'text-bg-primary'
          ...
          {/* Amount */}
          <div>
            <p className="text-xs font-medium text-text-secondary mb-2">Amount (oz)</p>
            {!showCustom ? (
              <div className="grid grid-cols-4 gap-2">
                {AMOUNT_PRESETS.map((a) => (
                  <button
                    key={a}
                    onClick={() => setAmount(a)}
                    className={`min-h-[48px] rounded-xl text-sm font-bold transition-all active:scale-95
                      ${amount === a
                        ? 'text-bg-primary'
                ...
                <button
                  ...
                  className="min-h-[48px] rounded-xl text-sm font-medium bg-white/5 text-text-secondary
              ...
              <div className="flex gap-2 items-center">
                <input
                  ...
                  placeholder="0.0"
                  autoFocus
                  className="flex-1 min-h-[48px] px-4 rounded-xl bg-white/5 text-text-primary text-center
                             text-lg font-mono border border-white/10 focus:outline-none
                             focus:border-white/20 placeholder:text-text-muted"
                />
                <span className="text-sm text-text-secondary font-medium">oz</span>
                <button
                  onClick={() => {
                    setShowCustom(false);
                    setCustomAmount('');
          ...
          <button
            ...
            className="min-h-[60px] rounded-xl text-base font-bold active:scale-95 transition-all text-bg-primary"
      ...
      {mode === 'breast' && (
        <div className="flex flex-col gap-5">
          ...
          <p className="text-xs font-medium text-text-secondary">Choose side to start timer</p>
          ...
          <div className="flex flex-col gap-3">
            {([
              ...
              <button
                ...
                className="flex items-center justify-center gap-3 min-h-[60px] rounded-xl
                           text-base font-semibold bg-white/5 text-text-primary
                           hover:bg-white/10 active:scale-[0.98] transition-all"
